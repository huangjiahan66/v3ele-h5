  ...
  <div class="search-view">
    ...
    <div class="search-view__history" v-if="!searchValue">
      ...
      <TransitionGroup name="list">
        <div class="history-tag" v-for="v in historyTags" :key="v" @click="handleTagClick(v)">
          {{ v }}
        </div>
        <div class="history-tag" @click="toggleHistoryTag" key="arrow">
          <VanIcon name="arrow-up" v-if="isHistoryTagShown"></VanIcon>
...
const searchState = ref(INIT) //是否正在搜索字段
const historyTags = computed(() =>
  isHistoryTagShown.value ? HISTORY_TAGS : HISTORY_TAGS.slice(0, 5)
)

console.log('historyTags=>', historyTags.value)

const handleSearch = async (v: string | number) => {
  try {
